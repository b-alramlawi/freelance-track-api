...
async function signupUser(req, res) {
    try {
        const {email, password, username, hourlyRate} = req.body;

        // Validate the request data against the registration schema
        const {error} = registrationSchema.validate({email, password, username, hourlyRate});
        ...
        const verificationLink = `${config.baseUrl}/api/auth/verify/${verificationToken}`;
...
async function verifyEmail(req, res) {
    try {
        ...
        if (error instanceof jwt.TokenExpiredError) {
            ...
            return apiResponse(res, 400, 'error', 'Token has expired', null, null);
        }

        if (error instanceof jwt.JsonWebTokenError) {
            // Handle other token-related errors
            return apiResponse(res, 400, 'error', 'Invalid token', null, null);
        }
